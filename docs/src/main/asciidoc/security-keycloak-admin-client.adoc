...
== Overcoming blocked thread when using a reactive pipeline

Some of the methods on the Keycloak admin client block the thread which result in an exception when you use the admin client in a reactive pipeline.

At the most basic level, you can deal with that at the entry-point 
== Testing
